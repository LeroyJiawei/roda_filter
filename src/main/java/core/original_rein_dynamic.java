...
public class original_rein_dynamic {
    ...
    private final static int ATTRIBUTE_NUM = 50;
    ...
    private static final String mtFile = "data/mt.txt";
    //dynamic version
    public static void main(String[] args) {
        Properties properties = new Properties();
        try {
            InputStream inputStream = new FileInputStream(new File("config/common.properties"));
        ...
        event.foreach((k,v) -> {
            ...
            if(eventNum == adjustSize) {
                ...
                if(!adjustSwitch && AverMatchTime > baseMatchTime * (1 - baseRate)){
                    LastMatchThread = match_thread_num;
                    match_thread_num = 2;
                    LastMatchTime = AverMatchTime;
                    adjustSwitch = true;
                }else if(AverMatchTime > baseMatchTime * (1 - baseRate)) {
                ...
                }else if(baseMatchTime * (1 - baseRate) > AverMatchTime){
                    if(match_thread_num > 1){
                        LastMatchThread = match_thread_num;
                        LastMatchTime = AverMatchTime;
                        match_thread_num--;
